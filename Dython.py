...
class temp():
    def define(name, start):
        b = True
        for i in list(temps):
            if list(i)[0] == str(name):
                b = False
                break
        if bool(b):
            temps.append([str(name), start])
    def get(name):
        for i in list(temps):
            if list(i)[0] == str(name):
                return list(i)[1]
    ...
    def menu(caption, options, noAnwser):
        ...
        while True:
            ...
            if mest == "int" or mest == "float":
                if mest == "float":
                    testAnwser = int(round(float(testAnwser)))
                optionAnswer = testAnwser
                if int(optionAnswer) >= 0 and int(optionAnswer) < int(len(options)):
                    return int(optionAnswer)
            elif mest == "str":
                if bool(noAnwser) and str(optionAnwser) == "":
                    return None
